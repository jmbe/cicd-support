...
# Fall back to deprecated GITLAB_PRIVATE_TOKEN name if GITLAB_READONLY_API_TOKEN not given
GITLAB_READONLY_API_TOKEN=${GITLAB_READONLY_API_TOKEN:-$GITLAB_PRIVATE_TOKEN}

# When testing locally, provide a personal access token with scope read_api (or api)
...
if [[ -z "$GITLAB_READONLY_API_TOKEN" ]]; then
    echo "GITLAB_READONLY_API_TOKEN not provided" >&2
    exit 1
fi
...
commit=$(curl --header "PRIVATE-TOKEN: ${GITLAB_READONLY_API_TOKEN}" -s "${url}" | jq -r '.[0].sha' 2>/dev/null)
